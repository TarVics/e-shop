...
import {CategoryTreeType, CategoryType} from "../../../interface";
...
const NavigationCategoryMenu: FC<NavigationCategoryMenuType> = ({category, wide}) => {
    const navigate = useNavigate();
    const menuRef = useRef(null);
    const {collections, categories} = useAppSelector(state => state.refsReducer);

    const items = refsService.getCategoryTree(categories).find(item => item.id === category.id)?.childCategories || [];
    // if (!Array.isArray(items)) return null;

    const collection = category.collection ? refsService.getCollectionById(collections, category.collection) : null;
    const banner = collection?.banner;

    const dataColumnsCount = ((banner && banner.column) ? 2 : 3) + Number(items.length > 3 && (wide || false));
    ...
    const dataColumns = items.reduce<Array<CategoryTreeType[]>>((acc, val, index) => {
    ...
    const realColumns = (collection && banner && banner.column) ? dataColumns.length + 1 : dataColumns.length;
    ...
    dataColumns.forEach((column, columnIndex) => {
        const data = column.map((category, categoryIndex) => {
            ...
            category.childCategories.forEach(item => {
                res.push(
                    <li key={item.id}>
                        ...
                        <>
                            <div className="hidden-sm hidden-xs">
                                ...
                                <SectionBanner
                                    ...
                                    <h3 className="white-color text-uppercase">{category.banner.name}</h3>
                                </SectionBanner>
                                {/* <!-- /banner --> */}
                            </div>
                            <hr/>
    ...
    if (collection && banner && banner.column) {
        links.push(
