...
const Pagination: React.FC<PaginationProps> = (
  ...
  useEffect(() => {
    ...
    const offset = Math.max(Math.min(
      Math.trunc(page / linkCount) * linkCount, pageCount - linkCount), 0);
  ...
  return (
    <ul className="store-pages">
      ...
      {pageInfo.offset > 0 ?
        <li>
          <a href="." onClick={e => onPageClick(e, -2)}><i className="fa fa-caret-left"></i></a>
        </li> : null
      }
      {pages}
      {pageInfo.offset + linkCount < pageCount ?
        <li>
          <a href="." onClick={e => onPageClick(e, -1)}><i className="fa fa-caret-right"></i></a>
        </li> : null
      }
    </ul>
  );
